 ...
 * Matches pt-tracker's api/data.ts pattern exactly. Never touches
 * Supabase's publishable/anon key or RLS policies — this function holds
 * the secret key (SUPABASE_SECRET_KEY, the modern equivalent of the older
 * service_role key) server-side only, and is itself gated by a shared
 * secret the client sends, so no Supabase credential is ever shipped to
 * the browser. localStorage stays the fast, offline-first copy; this is
 * just the durable one that survives the browser clearing its own storage.
 */
